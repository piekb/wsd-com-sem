...
def context_window(bucket_size, features):
	...
	for sent_id in dat['sentence'].unique():
		...
		# pad with "x"
		words = np.array(pad(words, bucket_size-2))
		...
		while True:
			...
			if sense != 'O':
				...
				out = out.append(pd.Series({'sentence':sent_id, 'word':main_word, 'sense':sense, 'context':stripped}), ignore_index=True)
				
				for f in features:
					out[f"{f}_context"].loc[len(out[f"{f}_context"])-1] = list(sentence[sentence['word'].isin(window_data)][f].values)
